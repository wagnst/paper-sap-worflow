...
Business One ist eine der drei \gls{sap}-Lösungen für \gls{sme}. Sie wird im \gls{ondemand}- oder Vor-Ort-Modell unterstützt. Wenn ein schneller Datenzugriff bereitgestellt werden muss läuft \gls{sap} Business One auch auf der In-Memory-Computing-Plattform \gls{sap} \gls{hana}.

\gls{sap} und seine Partner stellen für Business One über 550 Branchenlösungen mit vorkonfigurierten Workflows bereit. Somit implementiert der Kunde eine Lösung, die schon von vielen Unternehmen genutzt wird und senkt somit seine Risiken und Kosten.
...
Die \gls{sap} All-in-One Lösung bietet ein \gls{sap} \gls{erp} und \gls{sap} \gls{nw} für mittelständische Unternehmen schon ab 90.000 Euro.

All-in-One basiert auf vordefinierten, branchenspezifischen Geschäftsprozessen, welche sich durch langjährige Erfahrung in diesem Bereich entwickelt haben. Dadurch lässt sich das System schnell aufsetzen und erfordert keine unnötigen Kosten, ohne jedoch an Funktionalität zu verlieren, da die Geschäftsprozesse genau an die Bedürfnisse der Firma konfiguriert werden können.
